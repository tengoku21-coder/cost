...
def get_load_type_idx(month, hour, weekday):
    ...
    if weekday == 6: return 0 # 일요일 -> 경부하
    if weekday == 5 and base_idx == 2: return 1 # 토요일 최대->중간
    return base_idx
...
# 3. 메인 화면 UI
# ---------------------------------------------------------
st.set_page_config(page_title="충전 수익성 검증기 (v18.0)", layout="wide")

st.title("⚡ 충전 수익성 분석기 (검증 모드)")
st.markdown("##### 🔍 평균 단가가 왜 그렇게 나왔는지 **[산출 근거]**를 보여드립니다.")
    ...
    try:
        ...
        if st.button("🚀 정밀 분석 및 검증 시작"):
            with st.spinner('계산 검증 중...'):
                ...
                # 실제 원가 (모든 비용 포함)
                clean_df['실제원가(원/kWh)'] = clean_df.apply(lambda x: x['변동비_세후'] / x['판매_전력량'] if x['판매_전력량'] > 0 else 0, axis=1)
                ...
                # ----------------------------------------
                if total_sold_kwh > 0:
                    sum_var_cost = clean_df['변동비_세후'].sum() # 총 변동비
                    avg_var_cost = sum_var_cost / total_sold_kwh # 가중평균
                    
                    bep_cost = total_cost_bill / total_sold_kwh
                    max_cost = clean_df['실제원가(원/kWh)'].max()
                    min_cost = clean_df[clean_df['실제원가(원/kWh)'] > 10]['실제원가(원/kWh)'].min()
                    if pd.isna(min_cost): min_cost = 0
                else:
                    sum_var_cost = 0; avg_var_cost = 0; bep_cost = 0; max_cost = 0; min_cost = 0;
                ...
                st.divider()
                st.subheader("💡 1kWh당 핵심 단가 (검증용)")
                
                # 검증 패널 (Expandable)
                with st.expander("🔍 평균 전력량요금 산출 근거 보기 (클릭)", expanded=True):
                    st.write(f"**[공식]** 총 변동비(세금/손실포함) ÷ 총 판매량")
                    st.write(f"• 총 변동비: **{int(sum_var_cost):,}원**")
                    st.write(f"• 총 판매량: **{int(total_sold_kwh):,}kWh**")
                    st.write(f"• 결과: {int(sum_var_cost)} ÷ {int(total_sold_kwh)} = **{int(avg_var_cost)}원/kWh**")
                    st.info("※ 이 값은 '단순 평균'이 아니라, 실제 사용량을 반영한 '가중 평균'입니다.")

                k1, k2, k3, k4 = st.columns(4)
                k1.metric("평균 전력량요금", f"{int(avg_var_cost)}원/kWh", help="위 산출 근거를 확인하세요.")
                k2.metric("BEP 요금", f"{int(bep_cost)}원/kWh", delta="기본료 포함")
                k3.metric("최고 비싼 충전", f"{int(max_cost)}원/kWh")
                k4.metric("최고 싼 충전", f"{int(min_cost)}원/kWh")
                ...
                try:
                    st.dataframe(
                        display_df.style.format({
                            '판매량(kWh)': '{:.2f}', '매입량(손실반영)': '{:.2f}',
                            '실제원가(최종)': '{:.0f}', '매출': '{:,.0f}', '변동비총액': '{:,.0f}'
                        }).background_gradient(subset=['실제원가(최종)'], cmap='Reds'),
                ...
                st.download_button("📥 엑셀 다운로드", data=output.getvalue(), file_name="최종검증분석.xlsx")
